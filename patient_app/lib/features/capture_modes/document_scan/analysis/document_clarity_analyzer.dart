...
class _DocumentClarityParams {
  const _DocumentClarityParams(this.bytes, this.threshold);
...
DocumentClarityResult _analyzeDocumentClarityInIsolate(_DocumentClarityParams params) {
  try {
    ...
    return DocumentClarityResult(
      ...
      reason: isSharp
          ? null
          : 'Document looks blurry (clarity ${variance.toStringAsFixed(0)} < ${params.threshold.toStringAsFixed(0)}).',
    );
  } catch (_) {
...
class LaplacianDocumentClarityAnalyzer implements DocumentClarityAnalyzer {
  ...
  Future<DocumentClarityResult> analyze(File file) async {
    try {
      ...
      // Run CPU-intensive analysis in background isolate
      final params = _DocumentClarityParams(bytes, threshold);
      return await compute(_analyzeDocumentClarityInIsolate, params);
    } catch (_) {
      return const DocumentClarityResult(reason: 'Clarity analysis failed');
    }
  }
