...
class GoogleAuthService {
  ...
  Future<void> _ensureInitialized() async {
    ...
    if (kIsWeb) {
      ...
      try {
        final osVersion = Platform.operatingSystemVersion.split('\n').first;
        _log('Platform: ${Platform.operatingSystem} | OS version: $osVersion');
      } catch (_) {}
      final sanitized = _sanitize(_androidServerClientId);
      await GoogleSignIn.instance.initialize(
      ...
      if (sanitized.isEmpty) {
        ...
        if (!(sanitized.endsWith('.apps.googleusercontent.com') &&
            sanitized.contains('-'))) {
          _log(
            'WARN: serverClientId does not look like a Web OAuth client id (*.apps.googleusercontent.com)',
          );
        }
      }
    }
  ...
  Future<GoogleSignInAccount?> signIn() async {
    ...
    try {
      ...
      _log('authenticate error: code=${e.code}, message=$msg');
      if (msg.toLowerCase().contains('reauth failed') ||
          e.code.toString().toLowerCase().contains('canceled')) {
        _log(
          'HINT: Ensure the emulator has a signed-in Google account, Play services are up to date, and your account is added as a Test user in the OAuth consent screen.',
        );
      }
  ...
  Future<Map<String, String>?> getAuthHeaders({
    ...
    try {
      _log(
        'Fetching auth headers (promptIfNecessary: $promptIfNecessary)',
      );
      final headers =
      ...
      _log(
        'authorizationHeaders success; keys: ${headers.keys.join(', ')}',
      );
      return headers;
    } on GoogleSignInException catch (e, st) {
      _log('authorizationHeaders error: code=${e.code}, message=${e.toString()}');
      _log('STACK: ${st.toString().split('\n').first}');
      return null;
    } catch (e, st) {
  ...
  Future<String?> tryGetEmail() async {
    ...
    try {
      ...
      final GoogleSignInAccount? acc =
          await (fut ?? Future<GoogleSignInAccount?>.value(null));
      final String? email = acc?.email;
      if (email != null) {
        _log('Lightweight auth success for: $email');
      ...
      _log('Lightweight auth error: code=${e.code}, message=${e.toString()}');
      _log('STACK: ${st.toString().split('\n').first}');
      return null;
    } catch (e, st) {
  ...
  Future<String> diagnostics() async {
    ...
    if (!kIsWeb) {
      try {
        final osVersion = Platform.operatingSystemVersion.split('\n').first;
        buf.writeln('Platform: ${Platform.operatingSystem} | $osVersion');
      } catch (_) {}
      final cid = _sanitize(_androidServerClientId);
      buf.writeln(
        'ServerClientId set: ${cid.isNotEmpty} (tail: ...$androidServerClientIdTail)',
      );
    } else {
      buf.writeln('WebClientId set: ${_webClientId.isNotEmpty}');
    }
    ...
    try {
      ...
      final GoogleSignInAccount? acc =
          await (fut ?? Future<GoogleSignInAccount?>.value(null));
      final String? email = acc?.email;
      buf.writeln('Lightweight: ${email ?? 'null'}');
      if (email != null) _rememberEmail(email);
    } catch (e) {
    ...
    try {
      final GoogleSignInAccount acc =
          await GoogleSignIn.instance.authenticate();
      final String email = acc.email;
      buf.writeln('Authenticate: $email');
      _rememberEmail(email);
    } on GoogleSignInException catch (e) {
      buf.writeln('Authenticate EX: code=${e.code} msg=${e.toString()}');
    } catch (e) {
    ...
    try {
      ...
      final desc = headers == null
          ? 'null'
          : 'ok keys=${headers.keys.join(',')}';
      buf.writeln('AuthHeaders(email): $desc');
    } on GoogleSignInException catch (e) {
      buf.writeln('AuthHeaders(email) EX: code=${e.code} msg=${e.toString()}');
    } catch (e) {
    ...
    try {
      ...
      final desc = headers == null
          ? 'null'
          : 'ok keys=${headers.keys.join(',')}';
      buf.writeln('AuthHeaders(drive.appdata): $desc');
    } on GoogleSignInException catch (e) {
      buf.writeln(
        'AuthHeaders(drive.appdata) EX: code=${e.code} msg=${e.toString()}',
      );
    } catch (e) {
